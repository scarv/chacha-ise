...
void chacha20_block_ise3(uint32_t out[16], uint32_t const in[16])
{
    ...
    {
        //                                A          B            C              D
        CHACHA_HR_V3(a0,a2,a4,a6); // QR(x[1]||x[0], x[5]||x[4], x[ 9]||x[ 8], x[13]||x[12]) - column 1 & 0 
        CHACHA_HR_V3(a1,a3,a5,a7); // QR(x[3]||x[2], x[7]||x[6], x[11]||x[10], x[15]||x[14]) - column 3 & 2
        REPACK_STATE(t2,t3,a2,a3); //  5, 4, 7, 6-> 6, 5, 4, 7
        REPACK_STATE(t6,t7,a7,a6); // 13,12,15,14->12,15,14,13
        //                                A          B            C              D
        CHACHA_HR_V3(a0,t2,a5,t6); // QR(x[1]||x[0], x[6]||x[5], x[11]||x[10], x[12]||x[15]) - column 1 & 0 
        CHACHA_HR_V3(a1,t3,a4,t7); // QR(x[3]||x[2], x[4]||x[7], x[ 9]||x[ 8], x[14]||x[13]) - column 3 & 2
