...
In comparing the scalar implementation, the \VERB{Opt.} implementation gains a reduced instruction count to 63\% of the OpenSSL baseline instruction count.
...
Notably, the instruction count of the $V_4$ implementation is reduced to at least 29\% (resp. 19\%) of the \VERB{Opt.} (resp. the baseline) instruction count. In other words, the $V_4$ implementation achieves a $3.4\times$ (resp. $5.4\times$) speed-up compared to the \VERB{Opt.} (resp. the baseline) implementation. 
