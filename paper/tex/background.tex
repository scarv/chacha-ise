%!TEX root=../paper.tex
The ChaCha20 stream cipher is a 256-bit stream cipher, designed by D. J. Bernstein in 2008~\cite{Ber:08}.
...
\begin{algorithm}
	...
	\KwFn{$\ALG{ChaCha20Block}( S )$}{
		...
		\For{$i = 1$ {\bf upto} $10$} {
		/*Odd Round*/ \;
		$\ALG{ChaCha20QuarterRound}( X[0],~X[4],~X[8],~X[12] )$ \; 
		$\ALG{ChaCha20QuarterRound}( X[1],~X[5],~X[9],~X[13] )$ \; 
		$\ALG{ChaCha20QuarterRound}( X[2],~X[6],~X[10],X[14] )$ \; 
		$\ALG{ChaCha20QuarterRound}( X[3],~X[7],~X[11],X[15] )$ \;  
		/*Even round*/ \;
		$\ALG{ChaCha20QuarterRound}( X[0],~X[5],~X[10],X[15] )$ \; 
		$\ALG{ChaCha20QuarterRound}( X[1],~X[6],~X[11],X[12] )$ \; 
		$\ALG{ChaCha20QuarterRound}( X[2],~X[7],~X[8]~,X[13] )$ \; 
		$\ALG{ChaCha20QuarterRound}( X[3],~X[4],~X[9]~,X[14] )$ \;  
		}
		$K \ASN X + S$ \;
		$\KwRet{~K}$ \;
	}
