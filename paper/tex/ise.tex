...
%This section presents the design of ISE to accelerate the operation of the ChaCha block function. 
...
\INST{chacha.v1.ad    rd, rs1, rs2}{
%  uint32\xspace $a,b,c,d, ia, ib, ic,id, na,nd$; \;
  $a \ASN \GPR[*][{\VERB[ASM]{rs1}}]\{63\isep32\}; d \ASN \GPR[*][{\VERB[ASM]{rs1}}]\{31\isep0\};$ \;
  $b \ASN \GPR[*][{\VERB[ASM]{rs2}}]\{63\isep32\}; c \ASN \GPR[*][{\VERB[ASM]{rs2}}]\{31\isep0\};$ \;
  $a \ASN a + b; d \ASN (a \XOR  d) \LRT 16;$ \;
  $c \ASN c + d;~b \ASN (c \XOR  b) \LRT12;$ \;
  $na \ASN a+b; $ \;
  $nd \ASN (d \XOR na) \LRT 8;$ \;
  $\GPR[*][{\VERB[ASM]{rd}}] \ASN (na \LSH 32) \IOR nd;$ \;
}

\INST{chacha.v1.bc    rd, rs1, rs2}{
%  uint32\xspace $a,b,c,d, ib, ic,id, na,nd$; \;
  $a \ASN \GPR[*][{\VERB[ASM]{rs1}}]\{63\isep32\}; d \ASN \GPR[*][{\VERB[ASM]{rs1}}]\{31\isep0\};$ \;
  $b \ASN \GPR[*][{\VERB[ASM]{rs2}}]\{63\isep32\}; c \ASN \GPR[*][{\VERB[ASM]{rs2}}]\{31\isep0\};$ \;
  $c \ASN c + (a \XOR d \LRT 24); b \ASN (b \XOR c) \LRT 12;$ \;
  $nc \ASN c + d; $ \;
  $nb \ASN (b \XOR nc) \LRT 7;$ \;
  $\GPR[*][{\VERB[ASM]{rd}}] \ASN (nb \LSH 32) \IOR nc;$ \;
}
\end{itemize}

Each instruction takes two 64-bit-register operands \VERB[ASM]{rs1} and \VERB[ASM]{rs2}, each of which packs two 32-bit input elements of the quarter-round function. \VERB[ASM]{rs1} (resp. \VERB[ASM]{rs2}) contains two inputs $iA$ and $iD$ (resp. $iB$ and $iC$). \VERB{chacha.v1.ad} (resp. \VERB{chacha.v1.bc}) computes the outputs $oA$ and $oD$ (resp. $oB$ and $oC$) packed in its destination register \VERB[ASM]{rd}. 
...
\KwFn{$\ALG{QuarterRound}( X, Y )$}{
  \VERB[ASM]{chacha.v1.ad} R0, X, Y \;
  \VERB[ASM]{chacha.v1.bc} R1, R0, Y \;
  $\KwRet{\TUPLE{ R0, R1 }}$ \;
}
...
The first two ISEs (i.e., \VERB[ASM]{chacha.v2.ad0} and \VERB[ASM]{chacha.v2.bc0}) are used to compute the top half of the quarter round while the other two ISEs computer the bottom halve.
...
\begin{algorithm}
	...
	\BlankLine
	\KwFn{$\ALG{QuarterRound}( X, Y )$}{
		\VERB[ASM]{chacha.v2.ad0} X', X, Y \;
		\VERB[ASM]{chacha.v2.bc0} Y', X, Y \;
		\VERB[ASM]{chacha.v2.ad1} R0, X', Y' \;
		\VERB[ASM]{chacha.v2.bc1} R1, X', Y' \;
		$\KwRet{\TUPLE{ R0, R1 }}$ \;
	}
...
\INST{chacha.v3.add   rd, rs1, rs2}{
%  uint32\xspace $a1,a0, b1, b0, r1,r0$; \;
  $a_1 \ASN \GPR[*][{\VERB[ASM]{rs1}}]\{63\isep32\}; a_0 \ASN \GPR[*][{\VERB[ASM]{rs1}}]\{31\isep0\};$\;
  $b_1 \ASN \GPR[*][{\VERB[ASM]{rs2}}]\{63\isep32\}; b_0 \ASN \GPR[*][{\VERB[ASM]{rs2}}]\{31\isep0\};$\;
  $r_1 \ASN a_1 + b_1; r_0 \ASN a_0 + b_0;$ \;
  $\GPR[*][{\VERB[ASM]{rd}}] \ASN (r_1 \LSH 32) \IOR r_0;$ \;
}

\INST{chacha.v3.xorrol  rd, rs1, rs2, imm}{
%  uint32\xspace $a1,a0, b1, b0, r1, r0$; \;
  $Dec = \{ 16, 12, 8, 7 \}$ \;
  $a_1 \ASN \GPR[*][{\VERB[ASM]{rs1}}]\{63\isep32\}; a_0 \ASN \GPR[*][{\VERB[ASM]{rs1}}]\{31\isep0\};$\;
  $b_1 \ASN \GPR[*][{\VERB[ASM]{rs2}}]\{63\isep32\}; b_0 \ASN \GPR[*][{\VERB[ASM]{rs2}}]\{31\isep0\};$\;

  $r_1 \ASN (a_1 \XOR b_1)\LRT Dec[\VERB[ASM]{imm}];$ \; 
  $r_0 \ASN (a_0 \XOR b_0)\LRT Dec[\VERB[ASM]{imm}];$ \;
  $\GPR[*][{\VERB[ASM]{rd}}] \ASN (r_1 \LSH 32) \IOR r_0;$ \;
}

%\INST{chacha.v3.xorrol.16  rd, rs1, rs2}{
%	uint32\xspace $a1,a0, b1, b0, r1, r0$; \;
...
%\INST{chacha.v3.xorrol.12  rd, rs1, rs2}{
%  uint32\xspace $a1,a0, b1, b0, r1, r0$; \;
%  $a1 \ASN \GPR[*][{\VERB[ASM]{rs1}}] \RSH 32; a0 \ASN \GPR[*][{\VERB[ASM]{rs1}}];$ \;
%  $b1 \ASN \GPR[*][{\VERB[ASM]{rs2}}] \RSH 32; b0 \ASN \GPR[*][{\VERB[ASM]{rs2}}];$ \;
...
%\INST{chacha.v3.xorrol.8  rd, rs1, rs2}{
%  uint32\xspace $a1,a0, b1, b0, r1, r0$; \;
%  $a1 \ASN \GPR[*][{\VERB[ASM]{rs1}}] \RSH 32; a0 \ASN \GPR[*][{\VERB[ASM]{rs1}}];$ \;
%  $b1 \ASN \GPR[*][{\VERB[ASM]{rs2}}] \RSH 32; b0 \ASN \GPR[*][{\VERB[ASM]{rs2}}];$ \;
...
%\INST{chacha.v3.xorrol.7  rd, rs1, rs2}{
%  uint32\xspace $a1,a0, b1, b0, r1, r0$; \;
%  $a1 \ASN \GPR[*][{\VERB[ASM]{rs1}}] \RSH 32; a0 \ASN \GPR[*][{\VERB[ASM]{rs1}}];$ \;
%  $b1 \ASN \GPR[*][{\VERB[ASM]{rs2}}] \RSH 32; b0 \ASN \GPR[*][{\VERB[ASM]{rs2}}];$ \;
...
Furthermore, in Xor-then-Rotation instruction, the rotate amount \VERB[ASM]{imm} is encoded using only 2 instruction bits.
...
\KwFn{$\ALG{HalfRound}( X, Y, Z, W )$}{
  \VERB[ASM]{chacha.v3.add}     X', X,  Y     \;
  \VERB[ASM]{chacha.v3.xorrol}  W', W,  X', 0 \;
  \VERB[ASM]{chacha.v3.add}     Z', Z,  W'    \;
  \VERB[ASM]{chacha.v3.xorrol}  Y', Y,  Z', 1 \;
  \VERB[ASM]{chacha.v3.add}     R0, X', Y'    \;
  \VERB[ASM]{chacha.v3.xorrol}  R3, W', R0, 2 \;
  \VERB[ASM]{chacha.v3.add}     R2, Z', R3    \;
  \VERB[ASM]{chacha.v3.xorrol}  R1, Y', R2, 3 \;
...
%  uint32\xspace $r2l, r1l$; \;
  $h \ASN \GPR[*][{\VERB[ASM]{rs2}}]\{31\isep0\}; l \ASN \GPR[*][{\VERB[ASM]{rs1}}]\{31\isep0\};$ \;
  $\GPR[*][{\VERB[ASM]{rd}}] \ASN (h \LSH 32) \IOR l;$ \;
}

\INST{packh    rd, rs1, rs2}{
%  uint32\xspace $r2h, r1h$; \;
  $h \ASN \GPR[*][{\VERB[ASM]{rs2}}]\{63\isep32\}; l \ASN \GPR[*][{\VERB[ASM]{rs1}}]\{63\isep32\};$ \;
  $\GPR[*][{\VERB[ASM]{rd}}] \ASN (h \LSH 32) \IOR l;$ \;
}

\INST{packhl    rd, rs1, rs2}{
%  uint32\xspace $r2h, r1l$; \;
  $h \ASN \GPR[*][{\VERB[ASM]{rs2}}]\{63\isep32\}; l \ASN \GPR[*][{\VERB[ASM]{rs1}}]\{31\isep0\};$ \;
  $\GPR[*][{\VERB[ASM]{rd}}] \ASN (h \LSH 32) \IOR l;$ \;
}

\INST{packlh    rd, rs1, rs2}{
%  uint32\xspace $r2l, r1h$; \;
  $h \ASN \GPR[*][{\VERB[ASM]{rs2}}]\{31\isep0\}; l \ASN \GPR[*][{\VERB[ASM]{rs1}}]\{63\isep32\};$ \;
  $\GPR[*][{\VERB[ASM]{rd}}] \ASN (h \LSH 32) \IOR l;$ \; 
}
...
The implementation of $V_3$ is chosen for the ChaCha ISE submodule to report in the table because it gains the best trade-off between area overhead (see \REFTAB{tab:res:hardcost1}) and software performance (see \REFTAB{tab:res:sw:perf1}). 
As can be seen, 
