...
Moreover, Chacha block function performs 20 rounds alternate between odd and even rounds. Between rounds, we must then re-pack the registers in order to the odd and even rounds can be executed with the same round function in software. These instructions are very similar to the \VERB[ASM]{pack} instructions in the Bitmanip extension [\ref{}] yet provide more flexible. The packing assisted ISEs are described as follows:
...
\begin{table}
	\caption{The numbers of packing instructions are executed in four ISEs assisted Chacha's round functions}
	\label{tab:ise:pack}
	\begin{tabular}{lrrrr}
		\toprule            
...
The open source Yosys [\ref{}] synthesis tool are used with default settings to obtain post-synthesis circuit area overhead of the implemented modules in terms of NAND2 gate equivalents and circuit depths (in the form of gate delays). 
%While more abstract than providing exact area and frequency results for a particular ASIC standard cell library, it is much easier to reproduce while still providing meaningful results [\ref{}]. 
...
We opt for a well-known Rocket core \cite{} as the host core for Chacha ISEs. 
The Rocket core is a highly configurable RISC-V core which executes instructions using a 5-stage, in-order pipeline. 
We take advantage of this to configure for a core supporting RV64IMC instruction set, i.e., the 64-bit [RV:19a, Section 5]) base integer ISA plus standard Multiplication [RV:19a, Section 7] and Compressed [RV:19a, Section16] extensions. It is also configured to support an instruction cache, a data cache,and a branch prediction mechanism, but no floating-point support.
To support Chacha ISE, The Rocket core have additional configuration to support custom instructions, for which we choose th CUSTOM 0 opcode [\cite{}], to decode the Chacha ISE and have a Rocket Custom Coprocessor (RoCC) interface [\cite{}] which access Chacha ISE implementation submodules to execute instructions. Since the ISE variants' implementations 
...
\begin{table}
	...
	\begin{tabular}{lcc}
		...
		\midrule
		RV64 Rocket Core         &    98150 (100\%)     &  153  \\
		~~~~~|-- ALU             &     3719  (03.78\%)  &   28  \\
		~~~~~|-- Muldiv          &    17171  (17.49\%)  &   40  \\
		~~~~~|-- Others          &    77260  (78.71\%)  &    -  \\
		RV64 Rocket + Chacha ISE &    99767 (101.65\%)  &  153  \\ 
